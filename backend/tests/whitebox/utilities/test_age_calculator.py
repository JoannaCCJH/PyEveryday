...
Slimmed.  Keeps ``TestDisplays::test_display_age_info_no_exception`` which
currently FAILS — it documents a real SUT bug (key mismatch:
``calculate_age`` returns ``days_to_next_birthday`` but
``display_age_info`` reads ``days_to_birthday``).  Do NOT remove that test.
"""
